...
protected:
    ...
    //处理发送错误
    int process_senderror(Ogre4a_AppFrame *tmpbuf);
    ...
    //将一个发送的帧放入等待发送队列
    int put_frame_to_sendlist(Ogre4a_AppFrame *ogre_frame);
...
public:
    ...
    //读取配置信息
    static int get_configure(Zen_INI_PropertyTree &cfg_file);
    ...
    //将数据从端口发送数据
    static int process_send_data(Ogre4a_AppFrame *tmpbuf );
