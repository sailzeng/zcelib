/// @file       zerg_stat_define.h
/// @date       2012/07/21 14:02
///
/// @author     yunfeiyang
///
/// @brief      小虫监控项定义
///
#ifndef ZERG_STAT_DEFINE_H_
#define ZERG_STAT_DEFINE_H_

//监控的FEATURE_ID
enum ZERG_MONITOR_FEATURE_ID
{
    ZERG_SERVICE_STAT_BEGIN = 1000,
    ZERG_ACCEPT_PEER_COUNTER = 1001,                    //五分钟内已经Accept的PEER计数
    ZERG_ACCEPT_PEER_NUMBER = 1002,                     //正在Accept的PEER数量,绝对值
    ZERG_CONNECT_PEER_COUNTER = 1003,                   //五分钟内已经Connect的PEER计数
    ZERG_CONNECT_PEER_NUMBER = 1004,                    //正在Connect的PEER数量,绝对值
    ZERG_BUFFER_STORAGE_NUMBER = 1005,                  //BUFFER缓冲区的CHUNK个数,绝对值
    ZERG_SEND_FRAME_COUNTER = 1010,                     //五分钟内从发送管道取出放入发送队列的帧总数，
    ZERG_RECV_FRAME_COUNTER,                            //五分钟内成功接收放入接收管道的帧总数
    ZERG_RECV_BYTES_COUNTER,                            //五分钟内成功收到字节数总数,非精确值,因为每个PEER都是隔一段时间统计一次
    ZERG_SEND_BYTES_COUNTER,                            //五分钟内成功发送的字节总数,非精确值
    ZERG_RECV_SUCC_COUNTER,                             //五分钟内接受成功帧总数,,非精确值
    ZERG_SEND_SUCC_COUNTER,                             //五分钟内发送成功帧总数,非精确值
    ZERG_RECV_FAIL_COUNTER,                             //五分钟内接收失败的数据帧总数
    ZERG_SEND_FAIL_COUNTER,                             //五分钟内发送数据失败的总数
    ZERG_RECV_BLOCK_COUNTER,                            //五分钟内接收数据发生阻塞的总数
    ZERG_SEND_BLOCK_COUNTER,                            //五分钟内发送数据发生阻塞的总数

    ZERG_UDP_RECV_COUNTER,                              //五分钟内收到UDP数据的次数
    ZERG_UDP_SEND_COUNTER,                              //五分钟内发送UDP数据的次数
    ZERG_UDP_RECV_BYTES_COUNTER,                        //五分钟内收到UDP数据字节的总数
    ZERG_UDP_SEND_BYTES_COUNTER,                        //五分钟内内发送UDP数据字节的总数
    ZERG_UDP_RECV_FAIL_COUNTER,                         //五分钟内收到UDP数据错误次数
    ZERG_UDP_SEND_FAIL_COUNTER,                         //五分钟内发送UDP数据错误次数

    ZERG_RECV_PIPE_FULL_COUNTER,                        //五分钟内内接收数据管道满错误的计数,接收丢失数据个数
    ZERG_RECV_PIPE_FULL_NUMBER,                         //接收数据管道满错误的累计值
    ZERG_SEND_LIST_FULL_COUNTER,                        //五分钟内 TCP的发送队列满了，发送丢失数据个数
    ZERG_SEND_LIST_FULL_NUMBER,                         //TCP的发送队列满了的累计值

    ZERG_SEND_FRAME_COUNTER_BY_SVR_TYPE,                //五分钟内从发送管道取出放入发送队列的帧总数,分服务类型
    ZERG_RECV_FRAME_COUNTER_BY_SVR_TYPE,                //五分钟内成功接收放入接收管道的帧总数,分服务类型
    ZERG_SEND_FRAME_COUNTER_BY_CMD,                     //五分钟内从发送管道取出放入发送队列的帧总数,分命令字
    ZERG_RECV_FRAME_COUNTER_BY_CMD,                     //五分钟内成功接收放入接收管道的帧总数,分命令字
    ZERG_SEND_FAIL_COUNTER_BY_SVR_TYPE,
    ZERG_SEND_LIST_FULL_COUNTER_BY_SVR_TYPE,            // 发送队列满导致导致丢包的数量，分服务ID

    ZERG_SERVICE_STAT_END,                              // 后面随便加
};

// 统计配置信息
static const zce::STATUS_ITEM_WITHNAME ZERG_MONITOR_ITEMS[] =
{
    DEF_STATUS_ITEM(ZERG_ACCEPT_PEER_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_ACCEPT_PEER_NUMBER,zce::STATUS_STATICS::ABSOLUTE_VALUE),
    DEF_STATUS_ITEM(ZERG_CONNECT_PEER_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_CONNECT_PEER_NUMBER,zce::STATUS_STATICS::ABSOLUTE_VALUE),
    DEF_STATUS_ITEM(ZERG_BUFFER_STORAGE_NUMBER,zce::STATUS_STATICS::ABSOLUTE_VALUE),
    DEF_STATUS_ITEM(ZERG_SEND_FRAME_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_RECV_FRAME_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_RECV_BYTES_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_SEND_BYTES_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_RECV_SUCC_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_SEND_SUCC_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_RECV_FAIL_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_SEND_FAIL_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_RECV_BLOCK_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_SEND_BLOCK_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),

    DEF_STATUS_ITEM(ZERG_UDP_RECV_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_UDP_SEND_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_UDP_RECV_BYTES_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_UDP_SEND_BYTES_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_UDP_RECV_FAIL_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_UDP_SEND_FAIL_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),

    DEF_STATUS_ITEM(ZERG_RECV_PIPE_FULL_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_RECV_PIPE_FULL_NUMBER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_SEND_LIST_FULL_COUNTER,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(ZERG_SEND_LIST_FULL_NUMBER,zce::STATUS_STATICS::PER_FIVE_MINTUES),

    DEF_STATUS_ITEM(COMM_STAT_APP_ALIVE,zce::STATUS_STATICS::PER_FIVE_MINTUES),

    // 进程及系统监控上报项
    DEF_STATUS_ITEM(COMM_STAT_SYS_CPU_RATIO,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(COMM_STAT_PROCESS_CPU_RATIO,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(COMM_STAT_SYS_MEM_USE_RATIO,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(COMM_STAT_AVAILABLE_MEM_SIZE,zce::STATUS_STATICS::PER_FIVE_MINTUES),

    // app收到的包字节数
    DEF_STATUS_ITEM(COMM_RECV_BUS_POP_BYTES,zce::STATUS_STATICS::PER_FIVE_MINTUES),
    DEF_STATUS_ITEM(COMM_SEND_BUS_PUSH_BYTES,zce::STATUS_STATICS::PER_FIVE_MINTUES),

    // 往notify task队列中发送数据失败的量
    DEF_STATUS_ITEM(COMM_STAT_TASK_QUEUE_SEND_FAIL,zce::STATUS_STATICS::PER_FIVE_MINTUES),
};

// 上报数据项个数
static const unsigned int ZERG_MONITOR_NUMBER = ZCE_ARRAY_SIZE(ZERG_MONITOR_ITEMS);

#endif // ZERG_STAT_DEFINE_H_
