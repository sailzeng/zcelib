...
protected:
    ...
    //请求的事务UID
    unsigned int            req_user_id_ = 0;
    //这个事务的命令字
    unsigned int            req_command_ = 0;


    //请求事务的发送者
    soar::SERVICES_ID       req_snd_service_;
    //请求事务的代理
    soar::SERVICES_ID       req_rcv_service_;
    //请求事务的代理
    soar::SERVICES_ID       req_proxy_service_;

    //请求的事务ID
    unsigned int            req_trans_id_ = 0;
    //请求的的会话ID:这里是回填事务id，意思变了,其实没怎么使用
    unsigned int            req_backfill_trans_id_ = 0;

    //请求的IP地址是什么.
    unsigned int            req_ip_address_ = 0;
    //请求的FRAME的的选项
...
//请求发送消息的Service,
template <class T>
int FSM_Base::request_send_to_peer(unsigned int cmd,
                                           const soar::SERVICES_ID &rcv_svc,
                                           const T &info,
                                           unsigned int option)
{
...
//请求发送数据到代理服务器,用请求的USER ID发送
template< class T>
int FSM_Base::request_send_to_proxy(unsigned int cmd,
                                            const soar::SERVICES_ID &proxy_svc,
                                            unsigned short rcvtype,
                                            const T &info,
                                            unsigned int option)
...
template< class T>
int FSM_Base::response_sendback(unsigned int cmd,
                                        const T &info,
                                        unsigned int option)
{
...
//为啥要用2作为函数名称而不重载呢，你可以考虑一下,我不作答
template< class T>
int FSM_Base::response_sendback2(unsigned int cmd,
                                         unsigned int user_id,
                                         const T &info,
                                         unsigned int option)
{
...
//最完整的填写发送消息函数，
template< class T>
int FSM_Base::sendmsg_to_service(unsigned int cmd,
                                         unsigned int user_id,
                                         unsigned int transaction_id,
                                         unsigned int backfill_trans_id,
                                         const soar::SERVICES_ID &rcv_svc,
