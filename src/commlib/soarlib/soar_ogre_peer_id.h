...
#pragma pack (1)
/****************************************************************************************************
class  OGRE_PERR_ID 　SOCKET PEER信息,适应IPV4的代码。
****************************************************************************************************/
class SOARING_EXPORT OGRE_PERR_ID
{
public:
    //无效的SERVICE 类型
...
public:
    //构造,析构函数,默认为0
    explicit OGRE_PERR_ID(unsigned int peer_ip_address = INVALID_PERR_IP_ADDRESS, 
        unsigned short peer_port = INVALID_PERR_PORT);
    explicit OGRE_PERR_ID(const ZCE_Sockaddr_In &);
    //
    ~OGRE_PERR_ID();
    ...
    bool operator ==(const OGRE_PERR_ID &others) const;
    //比较不同函数,
    bool operator !=(const OGRE_PERR_ID &others) const;
    //有个排序需求,
    bool operator <(const OGRE_PERR_ID &others) const;
...
{
public:
    size_t operator()(const OGRE_PERR_ID &peer_info) const
    {
