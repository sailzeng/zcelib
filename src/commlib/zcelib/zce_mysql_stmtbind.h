...
* @note
*/
class ZCELIB_EXPORT STMT_BindIsNULL_Adaptor
{
    friend class ZCE_Mysql_STMT_Bind;
protected:
    //
    bool         is_null_;
public:
    STMT_BindIsNULL_Adaptor(bool isnull):
        is_null_(isnull)
    {
    };
    ~STMT_BindIsNULL_Adaptor()
    {
    };
};
...
public:
    ...
    //绑定一个参数
    int bind_one_param(size_t paramno, enum_field_types paramtype, bool bisnull, void *paramdata , unsigned long szparam = 0);
    ...
    ZCE_Mysql_STMT_Bind &operator << (STMT_BindIsNULL_Adaptor &);
