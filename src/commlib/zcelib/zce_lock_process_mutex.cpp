    ...
    lock_(NULL)
{
    ...
    ret = ZCE_OS::pthread_mutex_initex(lock_,
                                       true,
                                       recursive,
                                       true,
                                       mutex_name);
...
{
    int ret = 0;
    ret = ZCE_OS::pthread_mutex_destroy (lock_);
...
{
    int ret = 0;
    ret = ZCE_OS::pthread_mutex_lock(lock_);
...
{
    int ret = 0;
    ret = ZCE_OS::pthread_mutex_trylock(lock_);
...
{
    int ret = 0;
    ret = ZCE_OS::pthread_mutex_unlock(lock_);
...
{
    int ret = 0;
    ret = ZCE_OS::pthread_mutex_timedlock(lock_, abs_time);
