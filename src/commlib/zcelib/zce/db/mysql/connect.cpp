...
//执行SQL语句，功能全集，不对外使用
int connect::execute_i(std::string_view sqlcmd,
                       size_t* num_affect,
                       size_t* last_id,
...
int connect::execute(std::string_view sqlcmd,
                     size_t& num_affect,
                     uint64_t& last_id)
{
    return execute_i(sqlcmd, &num_affect, &last_id, nullptr, false);
}
...
{
    return execute_i(sqlcmd, &num_affect, nullptr, &res, true);
}

//再取一次结果，注意其调用的是mysql_use_result,num_affect对它无效

//但不推荐使用,一次取一行,交互太多
int connect::query(std::string_view sqlcmd)
{
    return execute_i(nullptr, nullptr, &res, false);
}

//SQL 执行命令，这个事一个基础函数，内部调用
int connect::stmt_query_i(size_t* num_affect,
                          size_t* last_id)
{
    int tmpret = 0;
...
//执行SQL语句,不用输出结果集合的那种
int connect::stmt_query(size_t& num_affect, size_t& last_id)
{
    return stmt_query_i(&num_affect, &last_id);
}

//执行SQL语句,SELECT语句,转储结果集合的那种,
int connect::stmt_query(size_t& num_affect)
{
    return stmt_query_i(&num_affect, nullptr);
}

//准备SQL,并且分析绑定的变量
int connect::stmt_prepare_bind(std::string_view sqlcmd,
                               zce::mysql::stmt_bind* bind_param,
...
int connect::fetch_next_result(zce::mysql::result& res, bool bstore)
{
