                                      ...
                                      std::vector <sockaddr_in6>& vector_addr6)
{
    ...
    for (; prc_node != nullptr; prc_node = prc_node->ai_next)
    {
        ...
        if (AF_INET == prc_node->ai_family)
        {
            memcpy(&in, prc_node->ai_addr, prc_node->ai_addrlen);
            vector_addr.push_back(in);
        }
    }
    ...
    for (; prc_node != nullptr; prc_node = prc_node->ai_next)
    {
        if (AF_INET6 == prc_node->ai_family)
        {
            memcpy(&in6, prc_node->ai_addr, prc_node->ai_addrlen);
            vector_addr6.push_back(in6);
        }
    }
}

int _getaddrinfo_result(const char* hostname,
                        const char* service,
                        size_t* ary_addr_num,
                        sockaddr_in ary_addr[],
                        size_t* ary_addr6_num,
                        ...
                        std::vector <sockaddr_in6>* vector_addr6)
{
    ...
    {
        getaddrinfo_result_to_addrvector(result,
                                         *vector_addr,
                                         *vector_addr6);
    }
    //释放空间
...
int getaddrinfo_addrary(const char* hostname,
                        ...
                        sockaddr_in6 ary_addr6[])
{
    return _getaddrinfo_result(hostname,
                               service,
                               ary_addr_num,
                               ary_addr,
                               ary_addr6_num,
...
int getaddrinfo_addrary(const char* hostname,
                        const char* service,
                        std::vector<sockaddr_in> &ary_addr,
                        std::vector <sockaddr_in6>& ary_addr6)
{
    return _getaddrinfo_result(hostname,
                               service,
                               nullptr,
