              ...
              unsigned char word_delim)
{
    ...
    std::ofstream write_file(writefile_name.c_str(), std::ios::out | std::ios::binary);
    //先读取文件，全部读取
    if (!write_file)
    {
        ZCE_LOG(RS_ERROR, "[framework] Write cvs file read fail[%s].",
                writefile_name.c_str());
        return -1;
    }
    ZCE_LOG(RS_INFO, "[framework] Write cvs file [%s] open success.file .",
            writefile_name.c_str());
