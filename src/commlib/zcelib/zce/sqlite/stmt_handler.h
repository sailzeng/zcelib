...
public:
    ...
    */
    SQLite_STMTHdl(SQLite_Handler *sqlite3_handler);
    /*!
    * @brief      析构函数
    */
    ~SQLite_STMTHdl();
...
public:
    ...
    */
    ...
    //!导出结果,列号自动++
    template <class value_type>
    SQLite_STMTHdl &operator >> (value_type &val)
    {
    ...
    //!bind绑定参数,列号自动++
    template <class bind_type>
    SQLite_STMTHdl &operator << (bind_type val)
    {
    ...
    SQLite_STMTHdl &operator << (const SQLite_STMTHdl::BIN_Param &val);
    SQLite_STMTHdl &operator << (const std::string &val);
