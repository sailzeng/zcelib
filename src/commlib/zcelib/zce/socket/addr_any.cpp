...
//设置地址的family
void addr_any::set_family(int family)
{
    if (family == AF_INET)
    {
        in4_addr_.sin_family = AF_INET;
...
//通过域名得到IP地址
int addr_any::getaddrinfo(const char* node_name,
...
int addr_any::getaddrinfo(int family,
...
//通过IP取得域名
int addr_any::getnameinfo(char* host_name,
...
//设置端口号，
void addr_any::set_port(uint16_t port)
{
    if (sockaddr_ptr_->sa_family == AF_INET)
    {
...
//取得端口号
uint16_t addr_any::get_port(void) const
{
    if (sockaddr_ptr_->sa_family == AF_INET)
    {
...
// 检查地址是否相等
bool addr_any::operator == (const addr_any& others) const
{
    //比较地址协议簇，地址
    if (sockaddr_ptr_->sa_family == AF_INET &&
...
// 检查地址是否不相等
bool addr_any::operator != (const addr_any& others_sockaddr) const
{
