                           ...
                           bool if_restore)
{
    ...
    if (if_restore == true)
    {
        ...
        if (chunk_head->size_of_mem_ != alloc_size(numnode, numchunk, szchunk) ||
            ...
            chunk_head->size_of_buf_ != szchunk)
        {
            return -1;
        }
    }
...
void chunk_pool::clear()
{
    ...
    for (std::size_t i = 0; i < chunkpool_head_->num_of_node_; ++i)
    {
        ...
        cachenode_base_[i].size_of_buf_ = 0;
...
bool chunk_pool::create_node(std::size_t szdata, std::size_t& node_index)
{
    ...
    for (; tmpszdata > chunkpool_head_->size_of_buf_; ++use_chunk)
    {
        tmpszdata -= chunkpool_head_->size_of_buf_;
    ...
    (*(cachenode_base_ + node_index)).size_of_buf_ = szdata;
...
void chunk_pool::destroy_node(const std::size_t node_index)
{
    ...
    *(chunkindex_base_ + chunkinx) = chunkpool_head_->free_chunk_head_;
    ...
    (*(cachenode_base_ + node_index)).size_of_buf_ = 0;
...
void chunk_pool::free(std::size_t& free_node, std::size_t& free_chunk, std::size_t& max_room)
{
    ...
    max_room = chunkpool_head_->usable_of_chunk_ * chunkpool_head_->size_of_buf_;
}
...
bool chunk_pool::check_enough(std::size_t szdata)
{
    if (chunkpool_head_->usable_of_node_ == 0 ||
        ...
        (chunkpool_head_->usable_of_chunk_) * chunkpool_head_->size_of_buf_ <= szdata)
    {
        return false;
    }
                           ...
                           std::size_t& node_index)
{
    ...
    while (tmpszdata > 0)
    {
        szonce = (tmpszdata > chunkpool_head_->size_of_buf_) ? chunkpool_head_->size_of_buf_ : tmpszdata;
        memcpy(chunkdata_base_ + chunkinx * chunkpool_head_->size_of_buf_, indata + szcpy, szonce);
        szcpy += szonce;
        tmpszdata -= szonce;
...
{
    return (*(cachenode_base_ + node_index)).size_of_buf_;
}

//得到某个NODE的尺寸,以及相应的chunk的数量
void chunk_pool::node_size(const std::size_t node_index, std::size_t& node_size, std::size_t& chunknum)
{
    node_size = (*(cachenode_base_ + node_index)).size_of_buf_;

    if (0 == (node_size % (chunkpool_head_->size_of_buf_)))
    {
        chunknum = node_size / (chunkpool_head_->size_of_buf_);
    }
    else
    {
        chunknum = node_size / (chunkpool_head_->size_of_buf_) + 1;
    }
}
...
{
    return chunkpool_head_->size_of_buf_;
}
...
{
    std::size_t chunkinx = (*(cachenode_base_ + node_index)).chunk_index_;
    std::size_t tmpszdata = (*(cachenode_base_ + node_index)).size_of_buf_;
    szdata = tmpszdata;
    ...
    {
        szonce = (tmpszdata >= chunkpool_head_->size_of_buf_) ? chunkpool_head_->size_of_buf_ : tmpszdata;
        memcpy(outdata + szcpy, chunkdata_base_ + chunkinx * (chunkpool_head_->size_of_buf_), szonce);
        szcpy += szonce;
        tmpszdata -= szonce;
...
void chunk_pool::get_chunk(const std::size_t node_index, std::size_t chunk_no, std::size_t& szdata, char* outdata)
{
    ...
    std::size_t size_of_chunk = chunkpool_head_->size_of_buf_;
                               ...
                               char* outdata)
{
    ...
    assert(data_start < (*(cachenode_base_ + node_index)).size_of_buf_);
#endif

    std::size_t szonce = 0;

    //tmpszdata 最后==0
    while (tmpszdata >= chunkpool_head_->size_of_buf_)
    {
        szonce = (tmpszdata > chunkpool_head_->size_of_buf_) ? chunkpool_head_->size_of_buf_ : tmpszdata;
        tmpszdata -= szonce;
        chunkinx = (*(chunkindex_base_ + chunkinx));
    ...
    //
    szdata = chunkpool_head_->size_of_buf_ - tmpszdata;
    memcpy(outdata, chunkdata_base_ + chunkinx * (chunkpool_head_->size_of_buf_), szdata);
}
...
{
    std::size_t chunkinx = (*(cachenode_base_ + node_index)).chunk_index_;
    std::size_t szofchunk = chunkpool_head_->size_of_buf_;
    std::size_t size_of_buf_ = (*(cachenode_base_ + node_index)).size_of_buf_;

#if defined _DEBUG || defined DEBUG
    assert(data_start < size_of_buf_);
#endif

    //不是最后一块
    if (size_of_buf_ / szofchunk != data_start / szofchunk)
    {
    ...
    {
        szdata = szofchunk - (data_start % szofchunk) -
            ((size_of_buf_ % szofchunk) == 0 ? 0 : (szofchunk - (size_of_buf_ % szofchunk)));
    }
    ...
    chunk_data_point = chunkdata_base_ + chunkinx * (chunkpool_head_->size_of_buf_) + (data_start % szofchunk);
}
};