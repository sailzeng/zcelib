...
public:
    ...
    void clear()
    {
        if (vptr_ptr_ != NULL)
        {
            for (size_t i = 0; i < lordring_size_; ++i)
            {
    ...
    {
        return lordring_size_;
    }
    ///返回空闲空间的大小
    inline size_t free() const
    {
        return lordring_capacity_ - lordring_size_;
    }

    ///返回队列的容量
    inline size_t capacity() const
    {
        return lordring_capacity_;
    }
    ...
    {
        //已经用的空间等于容量
        if (lordring_size_ == lordring_capacity_)
        {
            return true;
        }
    ...
    bool resize(size_t new_max_size)
    {
        ...
        //调整几个内部参数
        lordring_start_ = 0;
        lordring_capacity_ = new_max_size;
        //lordring_size_ 不变
    ...
    bool pop_front(_value_type &value_data)
    {
        ...
        vptr_ptr_[lordring_start_].~_value_type();
        lordring_start_ = (lordring_start_ + 1) % lordring_capacity_;
    ...
    bool pop_back(_value_type &value_data)
    {
        ...
        value_data = vptr_ptr_[(lordring_start_ + lordring_size_) % lordring_capacity_];
        vptr_ptr_[(lordring_start_ + lordring_size_) % lordring_capacity_].~_value_type();
        --lordring_size_;
        return true;
    }
    ...
    iterator end()
    {
        return iterator(lordring_start_, lordring_capacity_, vptr_ptr_, lordring_size_);
    }
...
protected:
    ...
    ///循环队列的长度，结束位置通过cycdeque_start_+lordring_size_%cycdeque_capacity_得到，思路仍然是前开后闭
    size_t                 lordring_size_ = 0;
    ///队列的长度，
