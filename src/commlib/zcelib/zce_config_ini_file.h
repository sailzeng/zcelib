...
* @version
* @date       2002年1月3日
* @brief      INI 文件的读写，和WINDOWS的API功能类似，
*             读写中会保证文件原来的格式和注释，不会破坏文件的格式。
* 
* @details    里面INI文件，一行的最大长度是8K
* 
* @note       这应该是2002年，2003年在计费项目时，自己在家里写的代码。
*             某种程度上，这个代码估计是不再会使用到了，
*             保留他某种程度是更多的怀旧。所以最后也还是把这些代码整理成了最新的
*             代码规范
*             整理注释发现里面还有WIN95的提示，呵呵
*/

#ifndef ZCE_LIB_INI_CONFIG_FILE_H_
#define ZCE_LIB_INI_CONFIG_FILE_H_

/*!
* @brief      INI文件读写的处理，接口都模仿WIndows的API
*/
class ZCELIB_EXPORT ZCE_INI_Cfgfile
{
...
public:
    ...
    ZCE_INI_Cfgfile(const char *file_name);
    //析构函数
    ~ZCE_INI_Cfgfile();
...
public:
    ...
    /*!
    * @brief      静态函数，读取INI文件的字段，段落，模仿WIN32接口GetPrivateProfileString,所有参数我就直接给你MSDN了
    * @return     size_t
    * If the function succeeds, the return value is the number of characters copied to the buffer, not including the terminating null character.
    * If neither sec_name nor key_name is NULL and the supplied destination buffer is too small to hold the requested string, the string is truncated and followed by a null character, and the return value is equal to size_ret_str minus one.
    * If either sec_name or key_name is NULL and the supplied destination buffer is too small to hold all the strings, the last string is truncated and followed by two null characters. In this case, the return value is equal to size_ret_str minus two.
    * @param      sec_name
    * Points to a null-terminated string that specifies the section containing the key name. If this parameter is NULL, the get_privateprofile_string function copies all section names in the file to the supplied buffer.
    * @param      key_name
    * Pointer to the null-terminated string containing the key name whose associated string is to be retrieved. If this parameter is NULL, all key names in the section specified by the sec_name parameter are copied to the buffer specified by the return_str parameter.
    * @param      default_str
    * Pointer to a null-terminated default string. If the key_name key cannot be found in the initialization file, get_privateprofile_string copies the default string to the return_str buffer. This parameter cannot be NULL.
    * Avoid specifying a default string with trailing blank characters. The function inserts a null character in the return_str
    * buffer to strip any trailing blanks.
    * @param      return_str
    * Pointer to the buffer that receives the retrieved string.
    * @param      size_ret_str
    * Specifies the size, in characters, of the buffer pointed to by the return_str parameter.
    * @param      file_name
    ...
    * @param      write_string
    * Points to a null-terminated string to be written to the file. If this parameter is NULL, the key pointed to by the key_name parameter is deleted.
    * @param      file_name
    ...
    * Remarks
    * The function searches the file for a key that matches the name specified by the key_name parameter under the section name specified by the sec_name parameter. A section in the initialization file must have the following form:
    * [section]
    * key=value
    *       .
    * The GetPrivateProfileInt function is not case-sensitive; the strings in sec_name and key_name can be a combination of uppercase and lowercase letters.
    */
    ...
    * @param   file_name
    * Points to a null-terminated string containing the name of the initialization file. If this parameter does not contain a full path for the file, the function searches the Windows directory for the file. If the file does not exist and file_name does not contain a full path, the function creates the file in the Windows directory. The function does not create a file if file_name contains the full path and filename of a file that does not exist.
    * @return bool
...
#endif //ZCE_LIB_INI_CONFIG_FILE_H_
