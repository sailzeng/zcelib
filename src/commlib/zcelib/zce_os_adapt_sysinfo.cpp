...
int read_proc_get_cpuhz (struct ZCE_SYSTEM_INFO *info)
{
    ...
    size_t read_len = 0;
    //读取/proc下的文件
    int ret = ZCE_OS::read_file_data(PROC_FILENAME_CPUHZ, buffer, sizeof(buffer), &read_len);
    ...
    char *out_para = NULL;
    in_para = ZCE_OS::skip_line(in_para);
    in_para = ZCE_OS::skip_line(in_para);
    in_para = ZCE_OS::skip_line(in_para);
    in_para = ZCE_OS::skip_line(in_para);
    in_para = ZCE_OS::skip_line(in_para);
...
static int read_proc_get_loadavg (struct ZCE_SYSTEM_PERFORMANCE *info)
{
    ...
    size_t read_len = 0;
    //读取/proc下的文件
    int ret = ZCE_OS::read_file_data(PROC_FILENAME_LOADAVG, buffer, sizeof(buffer), &read_len);
    ...
    in_para = out_para;
    in_para = ZCE_OS::skip_separator(in_para, '/');
...
int read_proc_get_meminfo(struct ZCE_SYSTEM_PERFORMANCE *info)
{
    ...
    size_t read_len = 0;
    //读取/proc下的文件
    int ret = ZCE_OS::read_file_data(PROC_FILENAME_MEMINFO, buffer, sizeof(buffer), &read_len);
    ...
    in_para = ZCE_OS::skip_token(in_para);
...
int read_proc_get_stat(struct ZCE_SYSTEM_PERFORMANCE *info)
{
    ...
    size_t read_len = 0;
    //读取/proc下的文件
    int ret = ZCE_OS::read_file_data(PROC_FILENAME_STAT, buffer, sizeof(buffer), &read_len);
    ...
    // 获得系统的uptime
    info->up_time_ = ZCE_OS::get_uptime();
...
int read_proc_get_uptime(struct ZCE_SYSTEM_PERFORMANCE *info)
{
    ...
    size_t read_len = 0;
    //读取/proc下的文件
    int ret = ZCE_OS::read_file_data(PROC_FILENAME_UPTIME, buffer, sizeof(buffer), &read_len);
...
int  ZCE_OS::read_proc_get_systemperf(struct ZCE_SYSTEM_PERFORMANCE *info)
{
    int ret = 0;
    ret = ZCE_OS::read_proc_get_loadavg(info);
    ...
    ret = ZCE_OS::read_proc_get_meminfo(info);
    ...
    ret = ZCE_OS::read_proc_get_stat(info);
...
int ZCE_OS::read_fun_get_systemperf(struct ZCE_SYSTEM_PERFORMANCE *zce_system_perf)
{
    struct sysinfo info;
    ...
    ret = ZCE_OS::read_proc_get_stat(zce_system_perf);
    ...
    zce_system_perf->up_time_ = ZCE_OS::get_uptime();
...
int ZCE_OS::get_system_info(ZCE_SYSTEM_INFO *zce_system_info)
{
    ...
    ret = ZCE_OS::read_proc_get_cpuhz(zce_system_info);
    if (ret != 0)
    {
        return ret;
    }
...
//得到操作系统的性能信息
int ZCE_OS::get_system_perf(ZCE_SYSTEM_PERFORMANCE *zce_system_perf)
{
#if defined ZCE_OS_WINDOWS
    ...
    zce_system_perf->idle_time_ = ZCE_OS::make_timeval(&idle_time);
    zce_system_perf->system_time_ = ZCE_OS::make_timeval(&kernel_time);
    zce_system_perf->user_time_ = ZCE_OS::make_timeval(&user_time);
