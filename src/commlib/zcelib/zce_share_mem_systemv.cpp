                               ...
                               const void *want_address)
{
    ...
    sysv_shmid_ = ZCE_OS::shmget(sysv_key, shm_size, shmget_flg);

    if (ZCE_INVALID_HANDLE == sysv_shmid_ )
    {
        ZLOG_ERROR("[zcelib] System memory shmget fail ,sysv key = %d,last error =%d. ", sysv_key, ZCE_OS::last_error());
        return -1;
    }

    shm_addr_ = ZCE_OS::shmat(sysv_shmid_, want_address, shmat_flg);

    if (shm_addr_ == MAP_FAILED)
    {
        ZLOG_ERROR("[zcelib] System memory shmat fail ,last error =%d. ", ZCE_OS::last_error());
        return -1;
    }
...
{
    return ZCE_OS::shmctl(sysv_shmid_, IPC_RMID, NULL);
}
