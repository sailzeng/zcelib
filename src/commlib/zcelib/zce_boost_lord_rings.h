...
*
* @details    BOOST也有类似的实现 circular_buffer，我这个地方没有偷师，殊途同归而已，
*             可以控制在数据满的情况下，是否覆盖,举一个应用的场景的例子，游戏服务器的用户的1s聊天队列，然后广播
*             你可以设置最大长度200，如果超出，后面的覆盖前面的，这样无论如何不会溢出。呵呵
*
* @note       原来取名cyc_deque，但是一天和Linker聊天他说说他写的类似的类名字是rings，我觉得这个
*             名字比较酷，所以选择这个名字，越来越觉得这个类好用。
*             2011年10月7日 做了一次修正，原来的实现是用空间+1标识前闭后开的空间，后来还是加入了size记录，
*             不需要再增加1个空间标识后开空间了。同时增加了迭代器
*
