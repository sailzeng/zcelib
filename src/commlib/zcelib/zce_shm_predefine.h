...
*             7.Date  : 20140308
*             Author  : Sailzeng  
*             Modification  ：处理原来代码中间的一些size_t结构，尽量用uint32_t代替他们，
*             （当然也有宏控制）
*             改进了原来HASH算法对于质数的管理方式，STL的方式过于粗放，并不见得适合服务器开发
*             我打算改进一下，
*
*               
...
*             第三，如果非要使用虚表，所有的数据在恢复的时候，最好重新new一次，恢复虚表指针，
* 
*/
...
//支持更大的数值定义，内部为了节省空间，一些结构的内部偏移是用uint32_t标识的，
//我估计内存过T的时候，可能会用到这个选项，其他时候都用不太上，
#define SUPPORT_BIGBIG_WORLD  0
