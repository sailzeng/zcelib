...
class ZCELIB_EXPORT ZCE_Thread_Condition  : public ZCE_Condition_Base
{
public:
    ...
    ZCE_Thread_Condition ()
    {
        ...
        ret = ZCE_OS::pthread_cond_initex(&lock_, false);

        if (0 != ret)
        {
            ZCE_TRACE_FAIL_RETURN(RS_ERROR, "ZCE_OS::pthread_cond_initex", ret);
            return;
        }
    ...
    {
        //销毁条件变量
        int ret =  ZCE_OS::pthread_cond_destroy(&lock_);
    ...
    {
        //
        int ret = ZCE_OS::pthread_cond_signal(&lock_);

        if (0 != ret)
        {
            ZCE_TRACE_FAIL_RETURN(RS_ERROR, "ZCE_OS::pthread_cond_signal", ret);
            return ;
        }
    }
    ...
    {
        //
        int ret = ZCE_OS::pthread_cond_broadcast(&lock_);

        if (0 != ret)
        {
            ZCE_TRACE_FAIL_RETURN(RS_ERROR, "ZCE_OS::pthread_cond_broadcast", ret);
            return;
        }
    }
