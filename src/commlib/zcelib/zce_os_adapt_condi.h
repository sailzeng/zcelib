...
*             对于模拟的实现，虽然我也测试过。但我实在不干保证什么，拿来当学术研究可以，真用？还是算了把。
*             你老上信号灯把。
* 
...
*             的等待者是否都已经被全部激活了。
* 
*             后面我自己高了一个简化的用事件模拟的版本，（同时只支持一种激发方式，而且外部锁要用MUTEX）
*             结果在测试中惨遭失败，
* 
*             以上答案分别是：
*             1.WHERE，
*             2.不是加锁，恰恰是解开，让其他人通行。
*             3.答案是未知，理论上是不用加的，而且理论上不加应该更快，但PTHREAD没有改出明确的说法，甚至
*             说过行为未定义，目前这里面的模拟实现实现和ACE一样，要求在signal，broadcast的时候，外部的锁
*             应该加上。
*             当然如果你用PTHRAD默认的实现和WINDOWS 2008的新同步原语，可以不加锁。
*             4.天知道。如果是模拟实现，由于要求外部锁要求加上，所以应该不会出现这种情况。
* 
...
* @return     int
* @param      cond
* @note       调用前，外部的锁要加上，(POSIX对这个问题表示含混，建议参考steven 的UNP V2 或者POSIX)
*/
