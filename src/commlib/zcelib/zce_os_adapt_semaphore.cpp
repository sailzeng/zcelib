...
int ZCE_OS::sem_destroy(sem_t *sem)
{
#if defined (ZCE_OS_WINDOWS)
...
//关闭信号灯
int ZCE_OS::sem_close(sem_t *sem)
{
#if defined (ZCE_OS_WINDOWS)
    ::CloseHandle(sem->sem_hanlde_);
    delete sem;
...
int ZCE_OS::sem_post (sem_t *sem, u_int release_count)
{
#if defined (ZCE_OS_WINDOWS)
...
int ZCE_OS::sem_trywait (sem_t *sem)
{
...
//信号灯超时锁定
int ZCE_OS::sem_timedwait(sem_t *sem, const ::timespec *abs_timeout_spec)
{
...
int ZCE_OS::sem_timedwait(sem_t *sem, const timeval *abs_timeout_val)
{
    assert(abs_timeout_val);
    //这个时间是绝对值时间，要调整为相对时间
    ::timespec abs_timeout_spec = ZCE_OS::make_timespec(abs_timeout_val);
    return ZCE_OS::sem_timedwait(sem, &abs_timeout_spec);
}
