...
//
int ZCE_OS::pthread_condattr_init(pthread_condattr_t *attr)
{
...
//条件变量对象的初始化
int ZCE_OS::pthread_cond_init(pthread_cond_t *cond,
                              const pthread_condattr_t *attr)
{
    ...
    ZCE_OS::pthread_mutexattr_init(&waiters_lock_attr);
    waiters_lock_attr.lock_shared_ = PTHREAD_PROCESS_PRIVATE;
    waiters_lock_attr.lock_type_ = PTHREAD_MUTEX_RECURSIVE;

    //初始化线程的互斥量
    int result = 0;
    result = ZCE_OS::pthread_mutex_init(&cond->simulate_cv_.waiters_lock_,
                                        &waiters_lock_attr);
    ...
    cond->simulate_cv_.block_sema_ = ZCE_OS::sem_open(sem_block_ptr,
                                                      O_CREAT,
    ...
    {
        ZCE_OS::pthread_mutex_destroy(&cond->simulate_cv_.waiters_lock_);
        return -1;
    }

    cond->simulate_cv_.finish_broadcast_ = ZCE_OS::sem_open(sem_finish_ptr,
                                                            O_CREAT,
                                                            ZCE_DEFAULT_FILE_PERMS,
                                                            0);
    ...
    {
        ZCE_OS::pthread_mutex_destroy(&cond->simulate_cv_.waiters_lock_);
        ZCE_OS::sem_close(cond->simulate_cv_.block_sema_);
        //其实没用
        //ZCE_OS::sem_unlink(sem_block_name);
        return EINVAL;
    }
...
int ZCE_OS::pthread_cond_initex(pthread_cond_t *cond,
                                bool win_mutex_or_sema)
{
    ...
    result = ZCE_OS::pthread_condattr_init (&attr);
    if (0 != result)
    {
        return result;
    }
    ...
    result = ZCE_OS::pthread_cond_init(cond, &attr);
    ZCE_OS::pthread_condattr_destroy (&attr);
...
//条件变量对象的销毁
int ZCE_OS::pthread_cond_destroy(pthread_cond_t *cond)
{
#if defined (ZCE_OS_WINDOWS)
    ...
    ZCE_OS::sem_close(cond->simulate_cv_.block_sema_);
    ZCE_OS::sem_close(cond->simulate_cv_.finish_broadcast_);

    //WIN平台下，无须调用这个函数，偷懒
    //ZCE_OS::sem_unlink(sem_name);
...
//条件变量等待一段时间，超时后继续
int ZCE_OS::pthread_cond_timedwait(pthread_cond_t *cond,
                                   ...
                                   const ::timespec *abs_timespec_out)
{
    ...
    if (ZCE_IS_USE_WIN2008_SIMULATE_PCV(cond))
    {
        ...
        {
            //得到相对时间，这个折腾，
            timeval now_time = ZCE_OS::gettimeofday();
            timeval abs_time = ZCE_OS::make_timeval(abs_timespec_out);

            timeval timeout_time = ZCE_OS::timeval_sub(abs_time, now_time, true);
            wait_msec = static_cast<DWORD>( ZCE_OS::total_milliseconds(timeout_time));
        }
    ...
    ZCE_OS::pthread_mutex_lock (&(cond->simulate_cv_.waiters_lock_));
    ++(cond->simulate_cv_.waiters_);
    ZCE_OS::pthread_mutex_unlock (&(cond->simulate_cv_.waiters_lock_));

    int result = 0;

    //对外部的锁重新解锁，
    //不对释放资源进行错误处理，如果释放失败，我能如何呢
    ZCE_OS::pthread_mutex_unlock (external_mutex);
    ...
    if (abs_timespec_out)
    {
        result = ZCE_OS::sem_timedwait(cond->simulate_cv_.block_sema_,
                                       abs_timespec_out);

    }
    else
    {
        result = ZCE_OS::sem_wait (cond->simulate_cv_.block_sema_);
    }

    //记录错误
    if (result != 0)
    {
        result = ZCE_OS::last_error_with_default(EINVAL);
    }
    ...
    if (result == 0)
    {
        ...
        if (last_waiter)
        {
            ...
            ZCE_OS::sem_post (cond->simulate_cv_.finish_broadcast_);
        }
    }

    //对外部的锁重新加上
    ZCE_OS::pthread_mutex_lock (external_mutex);
                                   ...
                                   const timeval *abs_timeout_val)
{
    ...
    //这个时间是绝对值时间，要调整为相对时间
    ::timespec abs_timeout_spec = ZCE_OS::make_timespec(abs_timeout_val);
    return ZCE_OS::pthread_cond_timedwait(cond,
                                          external_mutex,
                                          &abs_timeout_spec);
}

//条件变量等待
int ZCE_OS::pthread_cond_wait(pthread_cond_t *cond,
                              pthread_mutex_t *external_mutex)
{
#if defined (ZCE_OS_WINDOWS)
    //这样写是为了避免函数冲突告警，
...
//
int ZCE_OS::pthread_cond_broadcast(pthread_cond_t *cond)
{
#if defined (ZCE_OS_WINDOWS)
    ...
    if (have_waiters)
    {
        ...
        //唤醒所有的等待者,
        if ( ZCE_OS::sem_post (cond->simulate_cv_.block_sema_, cond->simulate_cv_.waiters_) != 0)
        {
            result = EINVAL;
        }
        ...
        else if (ZCE_OS::sem_wait (cond->simulate_cv_.finish_broadcast_) != 0 )
        {
            result = EINVAL;
        }
...
//发信号
int ZCE_OS::pthread_cond_signal(pthread_cond_t *cond)
{
#if defined (ZCE_OS_WINDOWS)
    ...
    if (have_waiters)
    {
        result = ZCE_OS::sem_post (cond->simulate_cv_.block_sema_);
