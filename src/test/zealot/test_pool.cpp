...
int test_share_ptr_pool(int /*argc*/, char* /*argv*/[])
{
    ...
    auto a = pool.alloc_shared();
    auto b = pool.alloc_shared();
    auto c = pool.alloc_shared();
    auto d = pool.alloc_shared();
    auto e = pool.alloc_shared();
    std::cout << "Pool size:" << pool.size() << " capacity:" << pool.capacity() << std::endl;
    *(a.get()) = 1;
    *(a.get()) = 2;
    *(a.get()) = 3;

    auto f = pool.alloc_shared();
    std::cout << "Pool size:" << pool.size() << " capacity:" << pool.capacity() << std::endl;
    auto g = pool.alloc_shared();
    auto h = pool.alloc_shared();
    auto i = pool.alloc_shared();
    auto j = pool.alloc_shared();
    std::cout << "Pool size:" << pool.size() << " capacity:" << pool.capacity() << std::endl;
    a.reset();
    b.reset();
    c.reset();
    ...
    e = pool.alloc_shared();
    std::cout << "Pool size:" << pool.size() << " capacity:" << pool.capacity() << std::endl;
    auto k = pool.alloc_shared();
    std::cout << "Pool size:" << pool.size() << " capacity:" << pool.capacity() << std::endl;
    auto l = pool.alloc_shared();
    auto m = pool.alloc_shared();
    auto n = pool.alloc_shared();
    auto o = pool.alloc_shared();
    std::cout << "Pool size:" << pool.size() << " capacity:" << pool.capacity() << std::endl;
